...
import { detectWallsFromImage } from './wallDetection.js'
...
export default function FloorPlanAnalyzer() {
  ...
  const analyze = async () => {
    ...
    try {
      // Step 1: Pixel-based wall detection (client-side)
      setLoadingStep('Detecting walls from pixels…')
      let pixCtx = null
      try {
        const pd = await detectWallsFromImage(image)
        setPixelData(pd)
        pixCtx = pd
        console.log('[CarpetPlan] Pixel detection:', {
          hWalls: pd.hWalls.length, vWalls: pd.vWalls.length,
          outline: pd.outline, gaps: pd.gapClassifications
        })
      } catch (pixErr) {
        console.warn('[CarpetPlan] Pixel detection skipped:', pixErr)
      }
  ...
  const drawOverlay = () => {
    ...
    const natW = imageNaturalSize.w, natH = imageNaturalSize.h
    const scX = dW / natW, scY = dH / natH
    const ctx = canvas.getContext('2d')
    ctx.clearRect(0, 0, dW, dH)
    const px = x => x * scX, py = y => y * scY
    const { doors = [], passages = [], windows = [], rooms = [] } = analysis

    // Building outline
    const outline = analysis.building_outline
    if (outline && outline.perimeter_corners && outline.perimeter_corners.length > 2) {
      ctx.strokeStyle = '#1D9E7566'; ctx.lineWidth = 2; ctx.setLineDash([8, 4])
      ctx.beginPath()
      const pts = outline.perimeter_corners
    ...
    if (showDimensions && rooms.length > 0) {
      rooms.forEach(room => {
        const bb = room.bounding_box; if (!bb) return
        const isHov = hoveredId === room.name
        ctx.strokeStyle = isHov ? '#378ADD' : '#378ADD66'
        ctx.lineWidth = isHov ? 2 : 1; ctx.setLineDash([6, 4])
        ctx.strokeRect(px(bb.x), py(bb.y), px(bb.w), py(bb.h)); ctx.setLineDash([])
        room.walls?.forEach(wall => {
          ...
          ctx.beginPath(); ctx.roundRect(tx - tw / 2, ty - 9, tw, 18, 3); ctx.fill()
          ctx.fillStyle = '#fff'; ctx.font = (isHov ? '600' : '500') + ' 11px sans-serif'
    ...
    // Doors
    if (showDoors) doors.forEach(door => {
      const isHov = hoveredId === door.id, isVert = door.wall_orientation === 'vertical'
      const avgScale = (scX + scY) / 2
      ctx.strokeStyle = isHov ? '#E24B4A' : '#E24B4Acc'; ctx.lineWidth = isHov ? 3 : 2.5
      ctx.setLineDash([5, 3]); ctx.beginPath()
      ctx.arc(px(door.arc_center_x), py(door.arc_center_y), door.arc_radius_px * avgScale,
        (door.arc_start_deg ?? 0) * Math.PI / 180, (door.arc_end_deg ?? 90) * Math.PI / 180)
      ctx.stroke(); ctx.setLineDash([])
      ctx.lineWidth = isHov ? 4 : 3
      const gx1 = px(door.gap_x), gy1 = py(door.gap_y)
      ...
      ctx.beginPath()
      if (isVert) { ctx.moveTo(gx1-4,gy1);ctx.lineTo(gx1+4,gy1);ctx.moveTo(gx2-4,gy2);ctx.lineTo(gx2+4,gy2) }
      else { ctx.moveTo(gx1,gy1-4);ctx.lineTo(gx1,gy1+4);ctx.moveTo(gx2,gy2-4);ctx.lineTo(gx2,gy2+4) }
      ctx.stroke()
      const lx = (gx1+gx2)/2, ly = (gy1+gy2)/2-14, tw = door.id.length*7+10
      ctx.fillStyle = isHov ? '#A32D2D' : '#E24B4A'
      ctx.beginPath(); ctx.roundRect(lx-tw/2,ly-9,tw,18,4); ctx.fill()
      ctx.fillStyle = '#fff'; ctx.font = 'bold 11px sans-serif'
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(door.id, lx, ly)
    })

    // Passages
    if (showPassages) passages.forEach(p => {
      const isHov = hoveredId === p.id, isVert = p.wall_orientation === 'vertical'
      ctx.strokeStyle = isHov ? '#1D9E75' : '#1D9E7599'; ctx.lineWidth = isHov ? 3 : 2
      ctx.setLineDash([4, 3])
      const gx1 = px(p.gap_x), gy1 = py(p.gap_y)
      const gx2 = isVert ? px(p.gap_x) : px(p.gap_x + p.gap_width_px)
      const gy2 = isVert ? py(p.gap_y + p.gap_width_px) : py(p.gap_y)
      ctx.beginPath(); ctx.moveTo(gx1,gy1); ctx.lineTo(gx2,gy2); ctx.stroke(); ctx.setLineDash([])
      const lx = (gx1+gx2)/2, ly = (gy1+gy2)/2-14, tw = p.id.length*7+10
      ctx.fillStyle = isHov ? '#0F6E56' : '#1D9E75'
      ctx.beginPath(); ctx.roundRect(lx-tw/2,ly-9,tw,18,4); ctx.fill()
      ctx.fillStyle = '#fff'; ctx.font = 'bold 11px sans-serif'
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(p.id, lx, ly)
    })

    // Windows
    if (showWindows) windows.forEach(w => {
      const isHov = hoveredId === w.id, isVert = w.wall_orientation === 'vertical'
      const cx = px(w.center_x), cy = py(w.center_y), halfW = px(w.width_px) / 2
      ctx.strokeStyle = isHov ? '#BA7517' : '#BA751799'; ctx.lineWidth = isHov ? 4 : 3
      ctx.beginPath()
      if (isVert) { ctx.moveTo(cx,cy-halfW);ctx.lineTo(cx,cy+halfW);ctx.moveTo(cx-5,cy-halfW);ctx.lineTo(cx+5,cy-halfW);ctx.moveTo(cx-5,cy+halfW);ctx.lineTo(cx+5,cy+halfW) }
      else { ctx.moveTo(cx-halfW,cy);ctx.lineTo(cx+halfW,cy);ctx.moveTo(cx-halfW,cy-5);ctx.lineTo(cx-halfW,cy+5);ctx.moveTo(cx+halfW,cy-5);ctx.lineTo(cx+halfW,cy+5) }
      ctx.stroke()
      const tw = w.id.length*7+10, labelY = isVert ? cy : cy-16
      ctx.fillStyle = isHov ? '#854F0B' : '#BA7517'
      ctx.beginPath(); ctx.roundRect(cx-tw/2,labelY-9,tw,16,3); ctx.fill()
      ctx.fillStyle = '#fff'; ctx.font = 'bold 10px sans-serif'
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(w.id, cx, labelY)
    })
  }

  const cal = analysis?.calibration, doors = analysis?.doors || []
  const passages = analysis?.passages || [], windows = analysis?.windows || []
    ...
    <div className="fpa-root">
      ...
      <main className="fpa-main">
        <div className="fpa-left">
          {!image ? (
            <div className="fpa-dropzone" onClick={() => fileInputRef.current?.click()}
            ...
            <div className="fpa-canvas-area">
              <div className="fpa-toggles">
                <button className={`fpa-toggle ${showDimensions?'on blue':''}`} onClick={()=>setShowDimensions(v=>!v)}><span className="tog-dot"></span> Dimensions</button>
                <button className={`fpa-toggle ${showDoors?'on red':''}`} onClick={()=>setShowDoors(v=>!v)}><span className="tog-dot"></span> Doors</button>
                <button className={`fpa-toggle ${showPassages?'on green':''}`} onClick={()=>setShowPassages(v=>!v)}><span className="tog-dot"></span> Passages</button>
                <button className={`fpa-toggle ${showWindows?'on amber':''}`} onClick={()=>setShowWindows(v=>!v)}><span className="tog-dot"></span> Windows</button>
                <button className="fpa-toggle reset" onClick={()=>{setImage(null);setAnalysis(null);setPixelData(null);setError(null);setImgReady(false)}}>Clear</button>
              </div>
              <div className="fpa-img-wrap">
                <img ref={imgRef} src={image} alt="Floor plan" className="fpa-img" onLoad={onImgLoad}/>
          ...
          {image && (
            <button className="fpa-analyze-btn" onClick={analyze} disabled={loading}>
              {loading ? (<><span className="spin"></span>{loadingStep}</>) : (
                <><svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6.5" stroke="currentColor" strokeWidth="1.5"/><path d="M8 5v3l2 2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>{analysis ? 'Re-analyze' : 'Analyze Floor Plan'}</>
              )}
            </button>
          )}
          {error && <div className="fpa-error"><strong>Analysis failed:</strong> {error}</div>}
        </div>
        {analysis && (
          <div className="fpa-right">
            ...
            <div className="fpa-tabs">
              {[{id:'doors',label:'Doors ('+doors.length+')',color:'red'},{id:'passages',label:'Passages ('+passages.length+')',color:'green'},{id:'windows',label:'Windows ('+windows.length+')',color:'amber'},{id:'rooms',label:'Rooms ('+rooms.length+')',color:'blue'}].map(t=>(
                <button key={t.id} className={`fpa-tab ${activeTab===t.id?'active '+t.color:''}`} onClick={()=>setActiveTab(t.id)}>{t.label}</button>
              ))}
            </div>
            <div className="fpa-panel">
              {activeTab==='doors'&&<div className="fpa-list">{doors.length===0&&<p className="fpa-empty">No swing doors detected</p>}{doors.map(d=>(<div key={d.id} className={`fpa-item door ${hoveredId===d.id?'hov':''}`} onMouseEnter={()=>setHoveredId(d.id)} onMouseLeave={()=>setHoveredId(null)}><div className="fpa-item-head"><span className="fpa-item-id red">{d.id}</span><span className="fpa-item-type">{d.type==='swing'?'Swing door':d.type}</span><span className="fpa-item-dim">{d.gap_width_m?.toFixed(2)}m</span></div><div className="fpa-item-loc">{d.location}</div><div className="fpa-item-meta">{d.connects?.join(' → ')}{d.wall_orientation&&<span className="fpa-chip">{d.wall_orientation}</span>}</div></div>))}</div>}
              {activeTab==='passages'&&<div className="fpa-list">{passages.length===0&&<p className="fpa-empty">No open passages detected</p>}{passages.map(p=>(<div key={p.id} className={`fpa-item passage ${hoveredId===p.id?'hov':''}`} onMouseEnter={()=>setHoveredId(p.id)} onMouseLeave={()=>setHoveredId(null)}><div className="fpa-item-head"><span className="fpa-item-id green">{p.id}</span><span className="fpa-item-type">Open passage</span><span className="fpa-item-dim">{p.gap_width_m?.toFixed(2)}m</span></div><div className="fpa-item-loc">{p.location}</div><div className="fpa-item-meta">{p.connects?.join(' → ')}{p.wall_orientation&&<span className="fpa-chip">{p.wall_orientation}</span>}</div></div>))}</div>}
              {activeTab==='windows'&&<div className="fpa-list">{windows.length===0&&<p className="fpa-empty">No windows detected</p>}{windows.map(w=>(<div key={w.id} className={`fpa-item window ${hoveredId===w.id?'hov':''}`} onMouseEnter={()=>setHoveredId(w.id)} onMouseLeave={()=>setHoveredId(null)}><div className="fpa-item-head"><span className="fpa-item-id amber">{w.id}</span><span className="fpa-item-type">Window</span><span className="fpa-item-dim">{w.width_m?.toFixed(2)}m</span></div><div className="fpa-item-loc">{w.location}</div><div className="fpa-item-meta">{w.wall_orientation} wall</div></div>))}</div>}
              {activeTab==='rooms'&&<div className="fpa-list">{rooms.length===0&&<p className="fpa-empty">No rooms detected</p>}{rooms.map((r,i)=>(<div key={i} className={`fpa-item room ${hoveredId===r.name?'hov':''}`} onMouseEnter={()=>setHoveredId(r.name)} onMouseLeave={()=>setHoveredId(null)}><div className="fpa-item-head"><span className="fpa-item-id blue">{r.label||'—'}</span><span className="fpa-item-type">{r.name}</span></div><table className="fpa-wall-table"><thead><tr><th>Wall</th><th>Length</th><th>Flags</th></tr></thead><tbody>{r.walls?.map(w=>(<tr key={w.side}><td>{w.side}</td><td><strong>{isNaN(w.length_m)?'?':w.length_m?.toFixed(2)}m</strong><span className="fpa-px"> ({Math.round(w.length_px||0)}px)</span></td><td>{w.has_door&&<span className="fpa-flag red">door</span>}{w.has_window&&<span className="fpa-flag amber">window</span>}</td></tr>))}</tbody></table></div>))}</div>}
            </div>
            {analysis.summary&&<div className="fpa-summary">{analysis.summary}</div>}
          </div>
        )}
      </main>
    </div>
