...
// ─── WALL DETECTION ──────────────────────────────────────────────────────────
// Key improvement: build a THICKNESS MAP first, then scan on that map.
// This separates thick wall lines from thin text/annotation lines.
...
function detectWalls(imageData, width, height) {
  ...
  // Step 2: Build VERTICAL thickness map for horizontal wall detection
  // For each pixel, count how many dark pixels are in a vertical column ±5
  // This tells us if a dark pixel is part of a thick wall or thin text
  ...
  // Step 3: Build HORIZONTAL thickness map for vertical wall detection
  ...
  // Step 4: Scan for horizontal walls using vThick map
  var hSegs = []
  var lastHY = -999
  for (var hy = 0; hy < height; hy++) {
    ...
    for (var hx = 0; hx < width; hx++) {
      var isWall = vThick[hy * width + hx] >= MIN_THICK
      ...
      if (isWall) {
        ...
        if (gap > MAX_GAP) {
          var end = hx - gap
          if (end - rs >= MIN_LEN) {
            hSegs.push({x1: rs, y1: hy, x2: end, y2: hy})
            found = true
          }
          rs = -1; gap = 0
        }
    ...
    if (rs !== -1) {
      ...
      if (endX - rs >= MIN_LEN) {
        hSegs.push({x1: rs, y1: hy, x2: endX, y2: hy})
        found = true
      }
    }
    if (found) lastHY = hy
  ...
  // Step 5: Scan for vertical walls using hThick map
  var vSegs = []
  var lastVX = -999
  for (var vx = 0; vx < width; vx++) {
    ...
    for (var vy = 0; vy < height; vy++) {
      var isWall2 = hThick[vy * width + vx] >= MIN_THICK
      ...
      if (isWall2) {
        ...
        if (gap2 > MAX_GAP) {
          var end2 = vy - gap2
          if (end2 - rs2 >= MIN_LEN) {
            vSegs.push({x1: vx, y1: rs2, x2: vx, y2: end2})
            found2 = true
          }
          rs2 = -1; gap2 = 0
        }
    ...
    if (rs2 !== -1) {
      ...
      if (endY - rs2 >= MIN_LEN) {
        vSegs.push({x1: vx, y1: rs2, x2: vx, y2: endY})
        found2 = true
      }
    }
    if (found2) lastVX = vx
  ...
  // Step 7: Envelope filter — remove segments outside building boundary
...
function mergeSegments(segs, axis, tol) {
  ...
  var s = segs.slice().sort(function(a, b) {
    return axis === 'h' ? a.y1 - b.y1 || a.x1 - b.x1 : a.x1 - b.x1 || a.y1 - b.y1
  })
  var m = [], c = {x1: s[0].x1, y1: s[0].y1, x2: s[0].x2, y2: s[0].y2}
  for (var i = 1; i < s.length; i++) {
    var si = s[i]
    var samePos = axis === 'h' ? Math.abs(si.y1 - c.y1) <= tol : Math.abs(si.x1 - c.x1) <= tol
    var overlaps = axis === 'h' ? si.x1 <= c.x2 + tol * 2 : si.y1 <= c.y2 + tol * 2
    if (samePos && overlaps) {
      if (axis === 'h') {
        c.x1 = Math.min(c.x1, si.x1); c.x2 = Math.max(c.x2, si.x2)
        c.y1 = Math.round((c.y1 + si.y1) / 2); c.y2 = c.y1
      } else {
        c.y1 = Math.min(c.y1, si.y1); c.y2 = Math.max(c.y2, si.y2)
        c.x1 = Math.round((c.x1 + si.x1) / 2); c.x2 = c.x1
      }
...
export default function FloorPlanAnalyzer() {
  ...
  var analyze = async function() {
    ...
    try {
      ...
      if (!ppm) {
        ...
        all.forEach(function(wl){mnX=Math.min(mnX,wl.x1,wl.x2);mxX=Math.max(mxX,wl.x1,wl.x2)})
        ppm = (mxX - mnX) / 10
      }

      setAnalysis({
        walls: wd.horizontal.concat(wd.vertical),
        hWalls: wd.horizontal, vWalls: wd.vertical,
        hCount: wd.horizontal.length, vCount: wd.vertical.length,
  ...
  var drawFloorPlan = function() {
    ...
    allW.forEach(function(w) {
      ...
      if (sl < 35) return
      var lb = (lp/ppm).toFixed(2)+'m'
      var fs = Math.max(8, Math.min(11, sl/8))
      ...
      {analysis && (<div className="fpa-result">
        ...
        <details className="fpa-details"><summary>Data</summary>
          <pre className="fpa-json">{JSON.stringify({ppm:analysis.ppm,rooms:(analysis.rooms||[]).length,
            matched:(analysis.rooms||[]).filter(function(r){return r.matchedDim}).map(function(r){return{dim:r.matchedDim.text,wpx:r.widthPx,hpx:r.heightPx,ppmX:Math.round((r.ppmX||0)*10)/10,ppmY:Math.round((r.ppmY||0)*10)/10}}),
